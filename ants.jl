...
function initialize_model(;
    number_ants::Int=125,
    dimensions::Tuple=(70, 70),
    diffusion_rate::Int=50,
    food_size::Int=7,
    random_seed::Int=2954,
    nest_size::Int=5,
    evaporation_rate::Int=10,
    pheromone_amount::Int=60,
    spread_pheromone::Bool=false,
    pheromone_floor::Int=5,
    pheromone_ceiling::Int=100
)
...
function antworld_step!(model)
    ...
    map!((x) -> x ≥ model.pheromone_floor ? x * (100 - model.evaporation_rate) / 100 : 0.0, model.pheromone_trails, model.pheromone_trails)
...
with_logger(debuglogger) do
    model = initialize_model(; number_ants=125, random_seed=6666, pheromone_amount=60, evaporation_rate=1)
    if !video
        params = Dict(
